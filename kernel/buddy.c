...
// Swithes bit at position index in array
void bit_switch(char* array, int index) {
  char b = array[index / 8];
  char m = 1 << (index % 8);
  array[index / 8] = (b ^ m);
}
...
void *bd_malloc(uint64 nbytes) {
  ...
  bit_switch(bd_sizes[k].alloc, blk_index(k, p) / 2);
  for (; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k - 1);  // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_switch(bd_sizes[k - 1].alloc, blk_index(k - 1, p) / 2);
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_switch(bd_sizes[k].alloc, bi / 2);           // free p at size k
...
void bd_mark(void *start, void *stop) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for (; bi < bj; bi++) {
      ...
      bit_switch(bd_sizes[k].alloc, bi / 2);
    }
  }
}
